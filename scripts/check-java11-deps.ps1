# Requires JDK 11+ on PATH and Maven. Fails if any runtime dep class is newer than Java 11 (major > 55).
...
Get-ChildItem -Path $outDir -Filter '*.jar' | Sort-Object Name | ForEach-Object {
  ...
  if ($maxMajor -gt 55) {
    Write-Host ("FAIL: {0} requires Java newer than 11 (major {1})" -f $jar.Name, $maxMajor)
    $failed = $true
  }
}

if ($failed) { exit 1 }
Write-Host 'OK: all runtime dependency classes are Java 11 or older.'
exit 0
